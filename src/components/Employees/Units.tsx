...
export default function Units() {
  ...
  const [editUnit, setEditUnit] = useState<{ id: number; name: string; manager: string } | null>(null);
  ...
  return (
    <div className="space-y-lg animate-fadeIn text-on-surface">
      {/* Header */}
      <div className="mb-sm">
        <h1 className="text-headline-md font-bold text-on-surface">Birimler</h1>
        <div className="flex items-center text-label-sm text-on-surface-variant gap-xs mt-1">
          <span>Kullanıcılar</span>
          <span>/</span>
          <span className="text-primary font-semibold">Birimler</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-lowest text-[11px] font-bold text-on-surface-variant uppercase tracking-wider border-b border-outline-variant/30">
                ...
                <th className="px-6 py-4">YÖNETİCİ</th>
            ...
            <tbody className="divide-y divide-outline-variant/20">
              {units.map((unit) => (
                <tr key={unit.id} className="hover:bg-surface-container-lowest transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm font-medium text-on-surface">{unit.manager}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm font-bold text-on-surface-variant">{unit.user_count} Kişi</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-0.5 text-[10px] font-bold text-emerald-700 bg-emerald-100 rounded-md uppercase">{unit.status}</span>
                  </td>
                      ...
                      <button 
                        onClick={() => {
                          useAppStore.getState().showDialog({
                            ...
                            onConfirm: () => {
                              useAppStore.getState().showDialog({ type: 'success', message: 'Birim silindi.' });
                            }
                          });
                        }}
        ...
        <div className="p-4 bg-surface-container-low flex justify-between items-center border-t border-outline-variant/30">
          <p className="text-sm font-medium text-on-surface-variant">Toplam {units.length} birim gösteriliyor</p>
      ...
      {isModalOpen && <NewUnitModal onClose={() => setIsModalOpen(false)} initialData={editUnit} />}
    </div>
  );
}
